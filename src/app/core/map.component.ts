...
import { Map, MapOptions, LatLngExpression, LatLngTuple, MarkerOptions} from 'leaflet';
...
export class MapComponent {
  ...
  ngOnInit() {
    let init_center: LatLngTuple = [1.1, 1.1, 0];

    if (this.location)
      init_center = <LatLngTuple>this.location.coordinates;

    this.map = L.map(this._element.nativeElement, <MapOptions>{
      /*zoomControl: false,*/
      center: L.latLng(init_center),
      //center: L.latLng([1.1, 1.1]),
  ...
  updateLocationMarker() {
    ...
    if (this.locationMarker)
      ...
      this.locationMarker =
        L.marker(this.location.coordinates, <MarkerOptions>{icon: MapComponent.icon, draggable: this.draggable})
          .addTo(this.map)
