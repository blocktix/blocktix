...
contract BlocktixEvent {
    ...
    // The start datetime of the event in UNIX Time
    uint public startTime;
    // The end datetime of the event in UNIX Time
    uint public endTime;
    // All tickets
    ...
    function BlocktixEvent() {
        organizer = msg.sender;
        name = 'Default';
        // Set defaults
        tickets.push(TicketType('Default', 100000000, 100, 0, now, now + 5 days));
    }
    ...
    function refundTicket(
        ...
        if (_ticketID < 0) { // Refund all tickets purchased by owner
            for (uint i=0; i<tickets.length; i++) {
                ...
                //TODO: Events
                paid = tickets[i].paid[owner];
                tickets[i].paid[owner] = 0;
                tickets[i].owners--;

                if(!contractAddress.send(paid - (paid / 20))) { // 5% refund fee
                    t.paid[owner] = paid;
                    t.owners++;
                }
            }
            ...
            if(!owner.send(paid - (paid / 20))) { // 5% refund fee
                t.paid[owner] = paid;
                t.owners++;
            }
        }
